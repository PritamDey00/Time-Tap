...
async function ensureStore() {
  try {
<<<<<<< HEAD
    const exists = await storage.exists('classrooms');
    if (!exists) {
      ...
      await storage.set('classrooms', [universalClassroom]);
    }
  } catch (error) {
    console.error('Error ensuring classroom store:', error);
    // Initialize with empty array if there's an error
    await storage.set('classrooms', []);
=======
...
<<<<<<< HEAD
  try {
    const classrooms = await storage.get('classrooms');
    return classrooms || [];
  } catch (error) {
...
<<<<<<< HEAD
  try {
    await storage.set('classrooms', classrooms);
  } catch (error) {
