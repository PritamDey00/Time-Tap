...
1. **Set up Redis Storage**: Create an Upstash Redis database for serverless-compatible storage
2. **Configure Environment Variables**: Set production environment variables (see [Environment Variables Guide](docs/ENVIRONMENT_VARIABLES.md))
3. **Deploy**: Deploy to your preferred platform

**Important**: This application automatically switches between filesystem storage (development) and Redis storage (production) based on the `NODE_ENV` environment variable.

## Notes

- **Storage**: 
  - **Development**: Uses local JSON files in `data/` directory
  - **Production**: Uses Redis storage via Upstash for Vercel compatibility
  - **Automatic Switching**: Application automatically selects appropriate storage based on environment
- **Security**: This is a production-ready application with proper environment configuration:
  - Use strong JWT secrets (see [Environment Variables Guide](docs/ENVIRONMENT_VARIABLES.md))
